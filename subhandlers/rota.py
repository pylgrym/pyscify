...
def setCookieImpl(name, val, expTime, response_headers):
...
def setCookie(name, val, expDeltaHours, response_headers):
	later = datetime.utcnow() + timedelta(hours=expDeltaHours)
	setCookieImpl(name,val,later,response_headers)
...
class MySubhandler(pysite.subhandlers.BaseHandler):
	...
	def init(self):
		...
		setCookie('myCook', 'roTarCook', 1, self.response_headers)
